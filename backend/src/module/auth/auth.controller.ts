...
import {
  RecaptchaValidationDto,
  RecaptchaValidationResultDto,
  DemoFormSubmissionDto,
} from './dto/recaptcha.dto';
import { OptionalRecaptcha } from './decorators/recaptcha.decorator';
...
export class AuthController {
  ...
  @ApiOperation({
    summary: '회원가입',
    description:
      '웹: CSRF Token 필수 | 모바일: X-Client-Type: mobile | reCAPTCHA: 선택적 검증',
  })
  @ApiResponse({
    status: 201,
    description: '회원가입 성공',
  ...
  @ApiResponse({
    status: 400,
    description: '잘못된 요청 또는 reCAPTCHA 검증 실패',
  })
  ...
  async login(
    ...
    const cookieOptions =
      this.authService.getRefreshTokenCookieOptions(clientType);
  ...
  @ApiOperation({
    summary: '토큰 갱신',
    description:
      '웹: 쿠키에서 Refresh Token 읽음 | 모바일: Bearer로 Refresh Token 전송',
  })
  @ApiHeader({
    name: 'X-Client-Type',
    description: '클라이언트 타입',
  ...
  async validateRecaptcha(
    ...
    const { RecaptchaStrategy } = await import(
      './strategies/recaptcha.strategy'
    );
  ...
  submitDemoForm(
    ...
    @Req()
    request: Request & { recaptchaResult?: RecaptchaValidationResultDto },
  ): DemoFormSubmissionDto {
